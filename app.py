...
if st.session_state['proses_selesai']:
    ...
    if supabase:
        try:
            # Import Plotly di dalam tab secara aman
            ...
            if res_tren.data:
                ...
                pilihan_ssr = st.selectbox("🎯 Filter Berdasarkan Lembaga SSR:", daftar_ssr, key="sb_tren_ssr")
                ...
                if not df_sumber.empty:
                    # Hitung distribusi frekuensi per indikator kesalahan
                    df_counts = df_sumber.groupby('indikator_kesalahan').size().reset_index(name='Jumlah Temuan')
                    
                    # ------------------------------------------------------
                    # PEMISAHAN DATA KOTAK A & KOTAK B
                    # ------------------------------------------------------
                    # Kotak B: Hanya yang mengandung kata 'konfirmasi'
                    df_box_b = df_counts[df_counts['indikator_kesalahan'].str.lower().str.contains('konfirmasi', na=False)]
                    df_box_b = df_box_b.sort_values(by='Jumlah Temuan', ascending=True) # Ascending agar di grafik horizontal urutan tertinggi di atas
                    
                    # Kotak A: Kecuali yang mengandung kata 'konfirmasi' (Ambil Top 10)
                    df_box_a = df_counts[~df_counts['indikator_kesalahan'].str.lower().str.contains('konfirmasi', na=False)]
                    df_box_a = df_box_a.sort_values(by='Jumlah Temuan', ascending=True).tail(10) # Ambil 10 terbesar
                    
                    st.markdown("<br>", unsafe_allow_html=True)
                    
                    # Membuat Layout 2 Kolom Sejajar
                    col_kotak_a, col_kotak_b = st.columns(2)
                    
                    # --- KOTAK A: INDIKATOR MUTLAK ---
                    with col_kotak_a:
                        st.markdown(f"##### 🟥 KOTAK A: Top {len(df_box_a)} Resume Indikator Mutlak (Non-Konfirmasi)")
                        if not df_box_a.empty:
                            fig_a = px.bar(
                                ...
                                orientation='h',
                                text_auto=True, # << MENAMPILKAN ANGKA DI BATANG GRAFIK
                                color_discrete_sequence=['#ef4444'] # Warna Merah Glassmorphism
                            )
                            fig_a.update_layout(
                    ...
                    with col_kotak_b:
                        st.markdown(f"##### 🟨 KOTAK B: Resume Indikator Tipe Konfirmasi")
                        if not df_box_b.empty:
                            fig_b = px.bar(
                                ...
                                orientation='h',
                                text_auto=True, # << MENAMPILKAN ANGKA DI BATANG GRAFIK
                                color_discrete_sequence=['#f59e0b'] # Warna Kuning/Amber Amber
                            )
                            fig_b.update_layout(
                                paper_bgcolor='rgba(0,0,0,0)',
                                plot_bgcolor='rgba(0,0,0,0)',
                    ...
                    st.info("Data tidak ditemukan untuk pilihan filter ini.")
            else:
                st.info("Belum ada rekam jejak log review tersimpan di database.")
        except Exception as e:
            st.error(f"Gagal memuat visualisasi resume: {e}")
