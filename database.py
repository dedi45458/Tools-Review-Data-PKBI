...
# ==========================================================
# FITUR LOG & VALIDASI REVIEW & MANAJEMEN DATABASE
# ==========================================================
def simpan_log_ke_neon(list_data_log):
    if not list_data_log: return False
    conn = dapatkan_koneksi_neon()
...
def import_data_HIV(df_HIV):
    ...
    for col in kolom_db:
        if col not in df_HIV.columns: df_HIV[col] = None 
...
def import_database_layanan(df):
    ...
    if not col_ssr or not col_layanan: return False, "Kolom 'Lembaga SSR/IU' atau 'Nama Layanan' tidak ditemukan."
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            query_insert = "INSERT INTO database_layanan (lembaga_ssr_iu, nama_layanan, jenis, kab_kota, kode_siha) VALUES (%s, %s, %s, %s, %s);"
...
def hitung_dan_ambil_log_db():
    ...
    dict_revisi = {}
    dict_justifikasi = {}
    if conn is None: return dict_revisi, dict_justifikasi
        
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            query = "SELECT lembaga_ssr, tanggal, id_klien, indikator_kesalahan_data, is_revisi, justifikasi FROM public.log_validasi_review;"
            cur.execute(query)
            rows = cur.fetchall()
            for row in rows:
...
def ambil_agregasi_terakhir_dari_neon():
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT nama_ssr, indikator_kesalahan, jumlah_kesalahan FROM agregasi_hasil_review_penjangkauan WHERE tanggal_review = %s", (target_date,))
            rows = cur.fetchall()
...
def simpan_detil_review_ke_neon(df_detil):
    if df_detil is None or df_detil.empty: return False
    conn = dapatkan_koneksi_neon()
    if not conn: return False
    ...
    try:
        with conn.cursor() as cur:
            ...
            for _, row in df_detil.iterrows():
                list_data.append((
                    batch_timestamp, str(row.get('Lembaga SSR', '')), str(row.get('Tanggal', '')),
                    str(row.get('ID Klien', '')), str(row.get('Kode Petugas', '')), str(row.get('Nama Kota', '')),
                    str(row.get('NIK', '')), str(row.get('Tipe Sasaran', '')),
                    str(row.get('Indikator Kesalahan Data', row.get('INDIKATOR KESALAHAN DATA', ''))),
                    str(row.get('Validasi Hasil Review', '')), str(row.get('Justifikasi', ''))
                ))
    ...
    try:
        with conn.cursor() as cur:
            ...
            df = pd.DataFrame(rows, columns=[
                "Lembaga SSR", "Tanggal", "ID Klien", "Kode Petugas", "Nama Kota", "NIK", 
                "Tipe Sasaran", "Indikator Kesalahan Data", "Validasi Hasil Review", "Justifikasi"
            ])
...
def ambil_status_storage_neon():
    ...
    try:
        with conn.cursor() as cur:
            ...
            return {
                "terpakai_mb": round(size_mb, 2), "sisa_mb": round(sisa_mb, 2),
                "total_mb": kuota_maksimal_mb, "persen_terpakai": round(persen_terpakai, 1)
            }
    except Exception as e:
        return None
    finally:
